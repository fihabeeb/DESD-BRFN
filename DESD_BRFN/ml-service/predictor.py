...
def predict(image_bytes: bytes) -> dict:
    ...
    return {
        ...
        "grade": grade,
        "breakdown": {"condition": condition_score, "color": color_score, "size": size_score},
        "labels": {"condition": condition, "colour": colour, "size": size},
        "confidences": {
            "condition": round(cond_conf, 1),
            "colour": round(col_conf, 1),
            "size": round(size_conf, 1),
        },
        # internal — used by main.py for gradcam; stripped before returning to caller
        "_img_resized": img_resized,
        "_img_input": img_input,
    }
