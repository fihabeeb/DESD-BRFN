"""
Sigmoid model v6 - Hard Negative Sampling.

Approach:
- For each training sample: 1 positive + 20 negatives FROM SAME CATEGORY
- This forces the model to learn finer distinctions between products
...
CATEGORY_NAMES = [
    'fruits', 'vegetables', 'dairy', 'meat', 'herbs_spices',
    'mushrooms', 'beverages', 'snacks', 'grains', 'preserves', 'honey_bee'
]
...
def get_product_category_mapping():
...
def build_sequences_with_hard_negatives(
    user_orders, product_to_idx, other_token, all_pids, 
    p2cat, max_hist=15, max_items=10, num_neg=20, seed=42
):
    np.random.seed(seed)
    ...
    for uid, orders in user_orders.items():
        ...
        for i in range(len(orders) - 1):
            ...
            for pidx in pos_idx:
                orig_pid = product_to_idx.get(pidx)
                if orig_pid:
                    pos_cat = p2cat.get(orig_pid, 'other')
                    
                    neg_pool = [p for p in products_by_cat.get(pos_cat, []) if p not in tgt[2]]
                    
                    if len(neg_pool) >= num_neg:
                        negs = np.random.choice(neg_pool, num_neg, replace=False)
                    else:
                        neg_pool_extra = [p for p in all_pids if p not in tgt[2]]
                        negs = list(neg_pool) + list(np.random.choice(
                            neg_pool_extra, 
                            min(num_neg - len(neg_pool), len(neg_pool_extra)),
                            replace=False
                        ))
...
def train_v6():
    print("=" * 50)
    print("V6: Hard Negative Sampling")
    print("=" * 50)
    ...
    print("\n[2b] Building category mapping...")
    p2cat = get_product_category_mapping()
    print(f"Products with categories: {len(p2cat)}")
    
    print("\n[3] Building sequences with hard negatives...")
    Xp, Xt, y, uids = build_sequences_with_hard_negatives(
        user_orders, p2i, 1, top, p2cat, 15, 10, 20
    )
    ...
    if True:
        print("\nSaving...")
        model.save("ml/recommendation/final/sigmoid_v6.keras")
        with open("ml/recommendation/final/sigmoid_v6_mappings.pkl", "wb") as f:
            pickle.dump({
                "p2i": p2i, 
                "i2p": i2p, 
                "u2c": u2c,
